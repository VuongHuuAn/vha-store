...
sellerRouter.post("/api/register-seller", auth, async (req, res) => {
  try {
    ...
    if (existingRequest) {
      return res
        .status(400)
        .json({ msg: "You already have a pending request" });
    }
    ...
    res.json({
      status: "pending",
      msg: "Seller request submitted successfully",
    });
  } catch (e) {
...
sellerRouter.post("/seller/delete-product", seller, async (req, res) => {
  try {
    ...
    if (!product) {
      return res
        .status(404)
        .json({ msg: "Product not found or you're not authorized" });
    }
    res.json(product);
  } catch (e) {
    res.status(500).json({ error: e.message });
...
sellerRouter.post("/seller/change-order-status", seller, async (req, res) => {
  try {
    ...
    if (!order) {
      return res
        .status(404)
        .json({ msg: "Order not found or you're not authorized" });
    }
    order.status = status;
    ...
    orders.forEach((order) => {
      order.products.forEach((item) => {
        if (item.product?.sellerId?.toString() === req.user) {
          ...
          const pricePerUnit = item.product.finalPrice || item.product.price;
          const earning = item.quantity * pricePerUnit;
          
          totalEarnings += earning;
          
          const category = item.product.category;
          if (!categoryData[category]) {
            categoryData[category] = { earning: 0, quantity: 0 };
          }
          categoryData[category].earning += earning;
          categoryData[category].quantity += item.quantity;
        }
      });
    });

    // Chuyển đổi categoryData thành mảng để phù hợp với format hiện tại
    const sales = Object.entries(categoryData).map(([category, data]) => ({
      category,
...
sellerRouter.post("/seller/update-product", seller, async (req, res) => {
  try {
    const { id, name, description, images, quantity, price, category } =
      req.body;
...
sellerRouter.get("/seller/addresses/cart", auth, async (req, res) => {
  try {
    const user = await User.findById(req.user);
    const sellerIds = [
      ...new Set(user.cart.map((item) => item.product.sellerId)),
    ];
...
sellerRouter.post("/seller/set-discount/:id", seller, async (req, res) => {
  try {
